...
# and PhysicalAI). Only the first two signals carry an unbounded physical unit
# and need scaling into a sensible range for the planner.
...
def normalize_egomotion(egomotion_history):
    ...
    Returns a NEW tensor. Normalizing in place would mutate the caller's batch,
    and because the scaling is not idempotent a second forward over the same
    batch would divide again — silently, with a plausible-looking result. The
    multi-seed inference loop in ``Platform/pipelines/overlay_precompute.py``
    reuses one batch across seeds and does exactly that.
    ...
    # One broadcast multiply over the whole history. The per-element Python
    # loop this replaces cost ~11 ms per forward on CUDA at batch 8 (1024
    # serialized scalar index ops), which is a measurable share of a step.
    scales = torch.ones(
        _EGOMOTION_SIGNALS,
